"""
完整生产环境测试：ScriptProcessingWorkflow（启用LLM）
...
async def test_full_production_with_llm():
    """
    完整生产环境测试（启用LLM）
    """
    print("=" * 80)
    print("🎬 ScriptProcessingWorkflow 完整生产环境测试（启用LLM）")
    print("=" * 80)
    print("使用真实数据: 末哥超凡公路")
    print("=" * 80)
    ...
    for ep in episodes_to_test:
        ...
        if os.path.exists(srt_path):
            ...
            print(f"  ✓ {ep}.srt - {file_size/1024:.1f} KB ({lines} 行)")
    ...
    # 项目名称
    project_name = "末哥超凡公路_production"
...
能活下来的人只能依靠序列超凡，不断迁徙。
    ...
    print(f"  - Hook检测: {config.enable_hook_detection} (仅ep01)")
    print(f"  - ABC分类: {config.enable_abc_classification}")
    print(f"  - 文本提取LLM: {config.text_extraction_provider}")
    print(f"  - 分段LLM: {config.segmentation_provider}")
    ...
    print(f"  - 平均耗时: {total_time/len(results):.1f} 秒/集")
...
async def main():
    ...
    主函数
    """
    results = await test_full_production_with_llm()
